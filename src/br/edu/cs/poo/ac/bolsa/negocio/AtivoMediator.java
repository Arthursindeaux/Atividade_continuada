...
public class AtivoMediator {
    ...
    private MensagensValidacao validar(Ativo ativo) {
        ...
        if (ativo.getValorMinimoAplicacao() > 0 && ativo.getValorMaximoAplicacao() > 0
                && ativo.getValorMinimoAplicacao() > ativo.getValorMaximoAplicacao())
            msgs.adicionar("Valor mínimo de aplicação deve ser menor ou igual ao valor máximo.");
        ...
        if (ativo.getTaxaMensalMinima() >= 0 && ativo.getTaxaMensalMaxima() >= 0
